      ...
      val sensalaReply: Pipe[F, WebSocketFrame, WebSocketFrame] =
        _.flatMap {
          case Text(discourseString, _) =>
            Json.parse(discourseString).validate[SensalaInterpretMessage] match {
            ...
            """.stripMargin
                ...
                val result1 =
                  Text(Json.toJson(StanfordParsed(convertTree(sentences.head))).toString)
                ...
                """.stripMargin
                    ...
                    val result2 =
                      Text(Json.toJson(SensalaError(s"Parsing failed: $error")).toString)
                ...
                """.stripMargin
                    ...
                    val result2 = Text(Json.toJson(SensalaParsed(convertNL(sentence))).toString)
                  ...
                  """.stripMargin
                        ...
                        result = Json.toJson(SensalaInterpreted(prettyTerm.pretty))
                      } yield Text(result.toString): WebSocketFrame
                    )
                }
              case JsSuccess(other, _) =>
                log.warn(s"Unexpected message: $other")
                Stream[F, WebSocketFrame](
                  Text(Json.toJson(SensalaError(s"Unexpected message: $other!")).toString)
                )
              case JsError(error) =>
                log.warn(s"Error while parsing json: $error")
                Stream[F, WebSocketFrame](
                  Text(Json.toJson(SensalaError(s"Error while parsing json: $error")).toString)
                )
            }
        }
